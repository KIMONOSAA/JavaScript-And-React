...
export function ChatgenerateResponse(props){
    ...
    fetch(API_URL,requestOptions).then(resolve => resolve.json()).then( data => {
        setmessAge([...messAge,[data.choices[0].message.content.trim()]]);
    }).catch( () => {
        setmessAge([...messAge,['error']]);
        setmessAge([...messAge,[data.choices[0].message.content.trim()]]);messageElement.textContent = "哼！ 出错啦 一定是你的密钥有问题唉嘿~";
    })
    return (
        <>
            <li className='imgbot'>
